...
// build query string that takes in user input from heuristics page
/*
 * TASKS:
    - get user threshold input
    - create api functionality for lower/higher temp threshold (similar
        to start/end time and also one for vent angle thresholds)
    - after getting JSON data, get data we need for each point that
        is an anomaly such as building, room, etc for table
    - display table
    - might have to have functions for updating user heuristic input?
        check lines 136, 158
    - maybe display number of points somewhere once user clicks search?
*/
function roomTempVentQueryBuilder(form_element, selector_list = SELECTOR_LIST) {

    $.getJSON(BACKEND_URL + "whatever we get from user")
}
